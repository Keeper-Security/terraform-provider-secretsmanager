...
func TestAccDataSourceSecretServerCredentials(t *testing.T) {
	...
	config := fmt.Sprintf(`
		...
		data "keeper_secret_server_credentials" "%v" {
			path = "%v"
			title = "%v"
		}
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(
						fmt.Sprintf("data.keeper_secret_server_credentials.%v", secretTitle),
						"type",
						secretType,
					),
					resource.TestCheckResourceAttr(
