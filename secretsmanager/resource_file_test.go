...
func TestAccResourceFile_create(t *testing.T) {
	secretType := "file"
	...
	config := fmt.Sprintf(`
		resource "secretsmanager_file" "%v" {
			folder_uid = "%v"
			uid = "%v"
			title = "%v"
			notes = "%v"
			file_ref {
				value { uid = "Tmk5WeXf0gzXk8ZLRVCR-g" }
				value { uid = "bZs6l8Hx9zkrRPYVFyuYLA" }
			}
		}
...
func TestAccResourceFile_update(t *testing.T) {
	secretType := "file"
	secretFolderUid := testAcc.getTestFolder()
	secretUid := core.GenerateUid()
	_, secretTitle := testAcc.getRecordInfo(secretType)
...
func TestAccResourceFile_deleteDetection(t *testing.T) {
	secretType := "file"
	secretFolderUid := testAcc.getTestFolder()
	secretUid := core.GenerateUid()
	_, secretTitle := testAcc.getRecordInfo(secretType)
...
func TestAccResourceFile_import(t *testing.T) {
	secretType := "file"
	secretFolderUid := testAcc.getTestFolder()
	secretUid := core.GenerateUid()
	_, secretTitle := testAcc.getRecordInfo(secretType)
