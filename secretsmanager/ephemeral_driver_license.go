...
var (
	_ ephemeral.EphemeralResource              = &ephemeralDriverLicense{}
	_ ephemeral.EphemeralResourceWithConfigure = &ephemeralDriverLicense{}
)

type ephemeralDriverLicense struct {
	meta providerMeta
}

type ephemeralDriverLicenseModel struct {
	Path                types.String `tfsdk:"path"`
	Type                types.String `tfsdk:"type"`
	Title               types.String `tfsdk:"title"`
	Notes               types.String `tfsdk:"notes"`
	DriverLicenseNumber types.String `tfsdk:"driver_license_number"`
...
func NewEphemeralDriverLicense() ephemeral.EphemeralResource {
	return &ephemeralDriverLicense{}
}

func (e *ephemeralDriverLicense) Metadata(_ context.Context, req ephemeral.MetadataRequest, resp *ephemeral.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_driver_license"
}

func (e *ephemeralDriverLicense) Schema(_ context.Context, _ ephemeral.SchemaRequest, resp *ephemeral.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Use this ephemeral resource to read a driver license record from Keeper Secrets Manager. Values are never stored in state.",
		Attributes: map[string]schema.Attribute{
			...
			"driver_license_number": schema.StringAttribute{
				Computed:    true,
...
func (e *ephemeralDriverLicense) Configure(_ context.Context, req ephemeral.ConfigureRequest, resp *ephemeral.ConfigureResponse) {
...
func (e *ephemeralDriverLicense) Open(ctx context.Context, req ephemeral.OpenRequest, resp *ephemeral.OpenResponse) {
	var data ephemeralDriverLicenseModel
	...
	if recordType != "driverLicense" {
		resp.Diagnostics.AddError("Record Type Mismatch",
			"record type '"+recordType+"' is not the expected type 'driverLicense' for this ephemeral resource")
		return
	}
	...
	data.DriverLicenseNumber = types.StringValue(secret.GetFieldValueByType("accountNumber"))
