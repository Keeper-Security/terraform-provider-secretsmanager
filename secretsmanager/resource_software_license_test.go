...
func TestAccResourceSoftwareLicense_create(t *testing.T) {
	...
	config := fmt.Sprintf(`
		resource "secretsmanager_software_license" "%v" {
			folder_uid = "%v"
			uid = "%v"
			title = "%v"
			notes = "%v"
			license_number {
				label = "MyLicenseNumber"
				required = true
				privacy_screen = true
				value = "MyLicenseNumber"
			}
			activation_date {
				label = "MyActivationDate"
				required = true
				privacy_screen = true
				value = 1651186276
			}
...
func TestAccResourceSoftwareLicense_update(t *testing.T) {
	secretType := "softwareLicense"
	secretFolderUid := testAcc.getTestFolder()
	secretUid := core.GenerateUid()
	_, secretTitle := testAcc.getRecordInfo(secretType)
