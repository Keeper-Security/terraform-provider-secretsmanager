...
var (
	_ ephemeral.EphemeralResource              = &ephemeralBirthCertificate{}
	_ ephemeral.EphemeralResourceWithConfigure = &ephemeralBirthCertificate{}
)

type ephemeralBirthCertificate struct {
	meta providerMeta
}

type ephemeralBirthCertificateModel struct {
...
func NewEphemeralBirthCertificate() ephemeral.EphemeralResource {
	return &ephemeralBirthCertificate{}
}

func (e *ephemeralBirthCertificate) Metadata(_ context.Context, req ephemeral.MetadataRequest, resp *ephemeral.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_birth_certificate"
}

func (e *ephemeralBirthCertificate) Schema(_ context.Context, _ ephemeral.SchemaRequest, resp *ephemeral.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Use this ephemeral resource to read a birth certificate record from Keeper Secrets Manager. Values are never stored in state.",
...
func (e *ephemeralBirthCertificate) Configure(_ context.Context, req ephemeral.ConfigureRequest, resp *ephemeral.ConfigureResponse) {
...
func (e *ephemeralBirthCertificate) Open(ctx context.Context, req ephemeral.OpenRequest, resp *ephemeral.OpenResponse) {
	var data ephemeralBirthCertificateModel
	...
	if recordType != "birthCertificate" {
		resp.Diagnostics.AddError("Record Type Mismatch",
			"record type '"+recordType+"' is not the expected type 'birthCertificate' for this ephemeral resource")
		return
	}
