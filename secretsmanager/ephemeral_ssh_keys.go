...
var (
	_ ephemeral.EphemeralResource              = &ephemeralSshKeys{}
	_ ephemeral.EphemeralResourceWithConfigure = &ephemeralSshKeys{}
)

type ephemeralSshKeys struct {
	meta providerMeta
}

type ephemeralSshKeysModel struct {
	Path       types.String `tfsdk:"path"`
	Type       types.String `tfsdk:"type"`
	Title      types.String `tfsdk:"title"`
	Notes      types.String `tfsdk:"notes"`
	Login      types.String `tfsdk:"login"`
	KeyPair    types.List   `tfsdk:"key_pair"`
...
func NewEphemeralSshKeys() ephemeral.EphemeralResource {
	return &ephemeralSshKeys{}
}

func (e *ephemeralSshKeys) Metadata(_ context.Context, req ephemeral.MetadataRequest, resp *ephemeral.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_ssh_keys"
}

func (e *ephemeralSshKeys) Schema(_ context.Context, _ ephemeral.SchemaRequest, resp *ephemeral.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Use this ephemeral resource to read SSH keys from Keeper Secrets Manager. Values are never stored in state.",
		Attributes: map[string]schema.Attribute{
			...
			"key_pair": schema.ListNestedAttribute{
				Computed:    true,
...
func (e *ephemeralSshKeys) Configure(_ context.Context, req ephemeral.ConfigureRequest, resp *ephemeral.ConfigureResponse) {
...
func (e *ephemeralSshKeys) Open(ctx context.Context, req ephemeral.OpenRequest, resp *ephemeral.OpenResponse) {
	var data ephemeralSshKeysModel
	...
	if recordType != "sshKeys" {
		resp.Diagnostics.AddError("Record Type Mismatch",
			"record type '"+recordType+"' is not the expected type 'sshKeys' for this ephemeral resource")
		return
	}
	...
	data.Passphrase = types.StringValue(secret.GetFieldValueByType("password"))

	keyPairList, diags := keyPairToListValue(ctx, secret)
	resp.Diagnostics.Append(diags...)
	data.KeyPair = keyPairList
